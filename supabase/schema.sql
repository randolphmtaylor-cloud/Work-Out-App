...
-- 3i. workout_goals and extensible goal-to-exercise links
...
CREATE POLICY "Users manage exercise links for own goals"
  ...
  USING (
    EXISTS (SELECT 1 FROM workout_goals goal WHERE goal.id = goal_exercise_links.goal_id AND goal.user_id = auth.uid())
  )
  WITH CHECK (
    EXISTS (SELECT 1 FROM workout_goals goal WHERE goal.id = goal_exercise_links.goal_id AND goal.user_id = auth.uid())
  );
...
INSERT INTO exercise_definitions
  (id, name, canonical_name, aliases, equipment_id, muscle_groups, tags, created_at)
VALUES
  ('goal-abs-leg-lifts', 'Lying Leg Lifts', 'lying-leg-lifts', ARRAY['lying leg raises','leg lifts'], NULL, ARRAY['core']::muscle_group[], ARRAY['core','isolation']::workout_tag[], '2024-01-01T00:00:00Z'),
  ('goal-abs-sit-ups', 'Sit-Ups', 'sit-ups', ARRAY['sit ups','situps'], NULL, ARRAY['core']::muscle_group[], ARRAY['core','isolation']::workout_tag[], '2024-01-01T00:00:00Z'),
  ('goal-abs-plank', 'Plank', 'plank', ARRAY['forearm plank'], NULL, ARRAY['core']::muscle_group[], ARRAY['core','isolation']::workout_tag[], '2024-01-01T00:00:00Z'),
  ('goal-abs-knee-raise', 'Hanging Knee Raise', 'hanging-knee-raise', ARRAY['hanging leg raise'], 'eq-4', ARRAY['core']::muscle_group[], ARRAY['core','compound']::workout_tag[], '2024-01-01T00:00:00Z'),
  ('goal-abs-cable-crunch', 'Cable Crunch', 'cable-crunch', ARRAY['rope crunch'], 'eq-6', ARRAY['core']::muscle_group[], ARRAY['core','isolation']::workout_tag[], '2024-01-01T00:00:00Z'),
  ('goal-glutes-hip-thrust', 'Hip Thrust', 'hip-thrust', ARRAY['barbell hip thrust'], 'eq-8', ARRAY['glutes','hamstrings']::muscle_group[], ARRAY['legs','lower','compound']::workout_tag[], '2024-01-01T00:00:00Z'),
  ('goal-glutes-rdl', 'Romanian Deadlift', 'romanian-deadlift', ARRAY['rdl'], 'eq-8', ARRAY['glutes','hamstrings','back']::muscle_group[], ARRAY['legs','lower','compound']::workout_tag[], '2024-01-01T00:00:00Z'),
  ('goal-glutes-bridge', 'Glute Bridge', 'glute-bridge', ARRAY['bridges'], NULL, ARRAY['glutes']::muscle_group[], ARRAY['legs','lower','isolation']::workout_tag[], '2024-01-01T00:00:00Z'),
  ('goal-glutes-kickback', 'Cable Kickback', 'cable-kickback', ARRAY['glute kickback'], 'eq-6', ARRAY['glutes']::muscle_group[], ARRAY['legs','lower','isolation']::workout_tag[], '2024-01-01T00:00:00Z')

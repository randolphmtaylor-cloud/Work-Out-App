    ...
    <div className="space-y-2.5">
      {data.map((item) => (
        <div key={item.equipment_id ?? item.equipment_name} className="flex items-center gap-3">
          <span className="text-xs text-zinc-600 dark:text-zinc-400 w-44 truncate shrink-0">{item.equipment_name}</span>
          ...
          <span className="text-xs text-zinc-400 dark:text-zinc-500 w-20 text-right shrink-0">
